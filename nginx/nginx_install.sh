...
[ ! -d /data ] && mkdir -p  /data/package
  ...
  if [ -e /bin/systemctl ]; then
   ...
   /bin/cp ../init.d/Nginx-init-CentOS /etc/init.d/nginx; sed -i "s@/usr/local/nginx@${nginx_install_dir}@g" /etc/init.d/nginx; chkconfig 0--add nginx; chkconfig nginx on;
