...
EOF
  ...
  echo -e "\033[33m [执行操作] 卸载文件上传工具lrzsz......\033[0m"
  yum remove -y lrzsz > /dev/null 2>&1
}
...
function ins_package() {
  ...
  yum install -y deltarpm gcc gcc-c++ make cmake autoconf libjpeg libjpeg-devel  > /dev/null 2>&1
  yum install -y libpng libpng-devel freetype freetype-devel libxml2 libxml2-devel  > /dev/null 2>&1
  yum install -y zlib zlib-devel glibc glibc-devel glib2 glib2-devel bzip2 bzip2-devel  > /dev/null 2>&1
  yum install -y ncurses ncurses-devel libaio readline-devel curl  > /dev/null 2>&1
  yum install -y curl-devel e2fsprogs e2fsprogs-devel krb5-devel libidn libidn-devel  > /dev/null 2>&1
  yum install -y openssl openssl-devel libxslt-devel libicu-devel libevent-devel libtool  > /dev/null 2>&1
  yum install -y libtool-ltdl bison gd-devel vim-enhanced pcre-devel zip unzip ntpdate  > /dev/null 2>&1
  yum install -y sysstat patch bc expect rsync git lsof  > /dev/null 2>&1
  yum install -y  install -y bind-utils vim wget lsof gcc gcc-c++ iftop vim openssl  > /dev/null 2>&1
  yum install -y lsof iftop htop net-tools ntpupdate  > /dev/null 2>&1
}
...
function set_timezone {
  ...
  /bin/cp -f /usr/share/zoneinfo/Asia/Shanghai /etc/localtime > /dev/null 2>&1
